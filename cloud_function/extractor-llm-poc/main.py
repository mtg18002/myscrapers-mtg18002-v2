...
def _vertex_extract_fields(raw_text: str) -> dict:
    """
    Ask Gemini to return JSON with exactly: price, year, make, model, mileage, cylinders, color, condition, transmission, fuel, and title_status. 
    """
    ...
    sys_instr = (
        ...
        "Search the entire input text, including title and description, before returning null. "
        "Prefer extracting a reasonable value over returning null when sufficient evidence exists. "
        "Return null only if no reasonable value can be determined. "
        "Rules: integers for price/year/mileage/cylinders; price in USD; mileage in miles; "
        "For cylinders: extract from patterns like 'V6', 'V8', 'I4', 'inline 4', '6 cyl', '4-cylinder', etc. "
            "If engine size is given (e.g., '2.0L', '3.5L'), infer: "
               " 2.0L–2.5L → typically 4 cylinders "
               " 3.0L–4.0L → typically 6 cylinders "
               " 5.0L+ → typically 8 cylinders "
            "Only infer if no explicit cylinder info is present. "
        "The transmission can be manual or automatic. If not explicitly stated, assume 'automatic' unless there is evidence of manual (e.g., 'stick', 'clutch', '5-speed'). "
        "Fuel can be gas, diesel, electric, or hybrid. Assume 'gas' unless the listing mentions electric/hybrid/diesel-related terms. "
        "For color, extract common colors (black, white, silver, gray, blue, red, green, etc.). Include variations (e.g., 'midnight black', 'pearl white', 'metallic gray'). Otherwise, return null. "
        "For condition, map phrases to categories: "
           " 'excellent', 'like new', 'mint' → excellent "
           " 'good', 'runs great', 'well maintained' → good "
           " 'fair', 'needs work', 'drives but...' → fair "
           " 'poor', 'mechanic special', 'not running' → poor. "
